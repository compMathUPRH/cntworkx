# este archivo creará el coarse grain con los ángulos, masas y eso
...
ldataobs = []
for tubo in tubos:    
    hex2pent.cycles_sets = set() #borra los anteriores
    ...
    cycle_length = 6
    print("Total cycles of length ",cycle_length," are ",hex2pent.countCycles(graph, cycle_length)) 
    c = hex2pent.getCycles()
    print('----------')
    ...
    hex2pent.cycles_sets = set() #borra los sets anteriores
    enlaces = pentatube.topologia.bonds
    enlaces = zip(enlaces['Atom1'],enlaces['Atom2'])
    g = nx.Graph(enlaces)
    
    cycle_length = 5
    print("Total cycles of length ",cycle_length," are ",hex2pent.countCycles(g, cycle_length)) 
    c = hex2pent.getCycles()
